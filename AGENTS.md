...
by tool-feeding changes, matching the flattened `skills/` convention (stab,
scoop, twirl, acquire, install already live there).

Recommended shape (from a prior brainstorm; not yet design-approved):
- Add `skills/tool_approach.py` → `ToolApproach(BaseSkill)` and
  `skills/tool_present.py` → `ToolPresent(BaseSkill)` (names TBD), reusing
  task_state tip geometry from the environment and `BaseSkill`'s twist/APF
  helpers.
...
Status: done — ToolApproach (skills/tool_approach.py) and ToolPresent
(skills/tool_present.py) landed; ApproachAndAlign and RetractObject are
